...
function wp_50_slider() 
{
	...
	$sSql = "select * from ".WP_PHOTO_50_TABLE." where wp_50_status='YES' and wp_50_type = %s ";
	if($wp_50_random == "YES")
	{
	...
	$sSql = $wpdb->prepare($sSql, $wp_50_type);
...
function wp_50_widget_init() 
{
	...
	{
		wp_register_sidebar_widget( 'wp-photo-text-slider-50', __('Wp photo text slider 50', 'wp-photo-text-slider-50'), 'wp_50_widget');
	}
	
	if(function_exists('wp_register_widget_control')) 	
	{
		wp_register_widget_control( 'wp-photo-text-slider-50', array( __('Wp photo text slider 50', 'wp-photo-text-slider-50'), 'widgets'), 'wp_50_control');
	} 
}
...
function wp_50_adminscripts() 
{
	if( !empty( $_GET['page'] ) ) 
	{
		switch ( $_GET['page'] ) 
		{
			case 'wp-photo-text-slider-50':
				wp_register_script( 'wp_50-adminscripts', plugins_url( 'pages/setting.js', __FILE__ ), '', '', true );
				wp_enqueue_script( 'wp_50-adminscripts' );
				$wp_50_select_params = array(
					'wp_50_path'   	=> __( 'Please enter the image path.', 'wp_50-select', 'wp-photo-text-slider-50' ),
					'wp_50_type'   	=> __( 'Please select gallery type, this is to group the images.', 'wp_50-select', 'wp-photo-text-slider-50' ),
					'wp_50_status' 	=> __( 'Please select the display status.', 'wp_50-select', 'wp-photo-text-slider-50' ),
					'wp_50_order' 	=> __( 'Please enter the display order, only number.', 'wp_50-select', 'wp-photo-text-slider-50' ),
					'wp_50_delete'	=> __( 'Do you want to delete this record?', 'wp_50-select', 'wp-photo-text-slider-50' ),
				);
				wp_localize_script( 'wp_50-adminscripts', 'wp_50_adminscripts', $wp_50_select_params );
				break;
		}
	}
}
...
add_action( 'admin_enqueue_scripts', 'wp_50_adminscripts' );
?>